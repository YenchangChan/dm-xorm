...
var (
	dmReservedWords = map[string]bool{
		"ACCESS":                    true,
		"ACCOUNT":                   true,
		"ACTIVATE":                  true,
		"ADD":                       true,
...
func (db *dm) IsReserved(name string) bool {
...
func (db *dm) SupportCharset() bool {
	return false
}

func (db *dm) SupportDropIfExists() bool {
	return false
}

func (db *dm) IndexOnTable() bool {
	return false
}

func (db *dm) DropTableSql(tableName string) string {
	return fmt.Sprintf("DROP TABLE `%s`", tableName)
}

func (db *dm) CreateTableSql(table *core.Table, tableName, storeEngine, charset string) string {
	var sql string
	sql = "CREATE TABLE "
	if tableName == "" {
...
/*func (db *dm) ColumnCheckSql(tableName, colName string) (string, []interface{}) {
...
func (db *dm) IsColumnExist(tableName, colName string) (bool, error) {
...
func (db *dm) GetColumns(tableName string) ([]string, map[string]*core.Column, error) {
	args := []interface{}{tableName}
...
func (db *dm) GetTables() ([]*core.Table, error) {
	args := []interface{}{}
...
func (db *dm) Filters() []core.Filter {
	return []core.Filter{&core.QuoteFilter{}, &core.SeqFilter{Prefix: ":", Start: 1}, &core.IdFilter{}}
}

type gdmDriver struct {
}

func (cfg *gdmDriver) Parse(driverName, dataSourceName string) (*core.Uri, error) {
	db := &core.Uri{DbType: "dm"}
...
//需要给xorm的core注册达梦
//go-xorm可以使用这种方法进行兼容
//xormplus需要实现的是dialects下的Driver接口，导致不兼容，但是相同方法可以让xormplus也支持
